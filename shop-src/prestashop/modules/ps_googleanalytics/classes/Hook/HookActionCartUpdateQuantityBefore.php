...
class HookActionCartUpdateQuantityBefore implements HookInterface
{
    ...
    public function run()
    {
		...
		if (!empty($product['specific_prices'])
			...
			&& (float) $product['specific_prices']['reduction'] > 0
		) {
			$isPromo = true;
		}
		...
		if (!$isPromo) {
			...
			if (isset($product['price_without_reduction'])) {
				$priceWithoutReduction = (float) $product['price_without_reduction'];
			} elseif (isset($product['price_without_reduction_without_tax'])) {
				$priceWithoutReduction = (float) $product['price_without_reduction_without_tax'];
			}

			$currentPrice = isset($item['price']) ? (float) $item['price'] : null;

			if ($priceWithoutReduction !== null && $currentPrice !== null && $priceWithoutReduction > $currentPrice) {
				$isPromo = true;
			}
		}
		...
		if ($this->params['operator'] === 'up' && $isPromo === true) {
			$promoJsCode = $this->module->getTools()->renderEvent(
				'add_to_cart_promo',
				$eventData
			);
			$this->module->getDataHandler()->persistData($promoJsCode);
		}
    }
