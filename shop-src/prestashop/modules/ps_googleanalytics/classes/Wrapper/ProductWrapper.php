...
{
    // 1) Normalize input so we can safely use array access below
    // ProductLazyArray / ProductListingLazyArray support array access, plain Product does not.
    if (is_object($product) && !($product instanceof \ArrayAccess)) {
        // Legacy Product object from product page hook
        if ($product instanceof \Product) {
            $idLang = (int) $this->context->language->id;

            // name can be string or array indexed by lang
            $name = '';
            ...
            // price (best effort): try getPrice if available, otherwise fallback to product->price
            $price = null;
            if (method_exists($product, 'getPrice')) {
                // getPrice() exists in legacy Product
            ...
            $product = [
                ...
                'manufacturer_name' => !empty($product->id_manufacturer)
                    ? Manufacturer::getNameById((int) $product->id_manufacturer)
                    : '',

                'id_category_default' => (int) ($product->id_category_default ?? 0),

                // These may be absent on product page hook; keep them optional
                'id_product_attribute' => (int) ($product->id_product_attribute ?? 0),
                'attributes_small' => $product->attributes_small ?? null,

                // default quantity for non-cart context
                'quantity' => 1,
            ];
        } else {
            // Unknown object type: try a generic cast (best effort)
    ...
    if (empty($product['price_amount'])) {
        $product['price_amount'] = $product['price'] ?? 0;
    }
    ...
    foreach ($productCategories as $key => $category) {
        $catId = is_array($category) ? ($category['id_category'] ?? null) : ($category->id_category ?? null);
        $defaultCat = $product['id_category_default'] ?? null;

        if ($catId !== null && $defaultCat !== null && (int) $catId === (int) $defaultCat) {
    ...
    foreach ($productCategories as $productCategory) {
        $catId = is_array($productCategory) ? ($productCategory['id_category'] ?? null) : ($productCategory->id_category ?? null);
        $catName = is_array($productCategory) ? ($productCategory['name'] ?? '') : ($productCategory->name ?? '');

        // Skip home category if we have more than 1 category
        if (count($productCategories) > 1 && $catId !== null && (int) $catId === (int) $this->homeCategory) {
            continue;
        }

        $item[$counter == 1 ? 'item_category' : 'item_category' . $counter] = (string) $catName;
